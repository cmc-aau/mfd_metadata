...
ontology.path <- paste0(wd, '/../../data/ontology/2023-06-08_mfd-habitat-ontology.xlsx')
...
filled.table <- readr::read_delim(file = paste0(metadata.path, "/", pid, "/230524_SustainScapes_MFD_HabitatClasses_ForVibeke.txt"),
  ...
  mutate(fieldsample_barcode = CORRECTED_barcode_fieldsample,
         ...
         correct_habitat_typenumber = case_when(hab_code == "2" ~ "2000",
                                                hab_code == "91" ~ "91E0",
                                                .default = hab_code),
...
mfd_db <- mfd_db %>%
  ...
  select(-correct_latitude,
         ...
         -correct_sitename,
         -correct_habitat_typenumber) %>%
  filter(!fieldsample_barcode %in% c("MFD10676", "MFD06329", "MFD06329")) # Duplicated samples as of 2023.06.07
