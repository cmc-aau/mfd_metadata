...
```{r, echo=eval_debug}
samples_on_map(data_table = mfd_db_subproject)
```

The samples from the `r pid` project were collected in the following locations:
```{r, echo=eval_debug}
tstats(project = pid, data = mfd_db_subproject)
```

The samples from the `r pid` project were collected in the following locations:
```{r, echo=eval_debug}
hstats(project = pid, data = mfd_db_subproject)
```
...
```{r, echo=eval_debug, eval=eval_internal}
missing_meta <- missing_metadata(project = pid, data_table = mfd_db_subproject)
...
```{r, echo=eval_debug}
rm(pid, des, ext, peo, res, com, mfd_db_subproject, N)
```
