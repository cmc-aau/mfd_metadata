...
ontology.path <- paste0(wd, '/../../data/ontology/2023-12-21_mfd-habitat-ontology.xlsx')
...
release_indices <- c("project_id",
                     ...
                     "coords_reliable", "cell.10km", "cell.1km") # NA is interpreted as all
...
mfd_db <- mfd_db %>%
  ...
  mutate(mfd_areatype       = ifelse(project_id == pid & habitat_type == "natural_soil", "Natural", mfd_areatype),
         ...
         mfd_sampletype     = ifelse(project_id == pid, "Soil", mfd_sampletype),
         habitat_typenumber = ifelse(project_id == pid &  str_detect(sampling_comment, "roadverge"), "1000", habitat_typenumber),
         sampling_date      = ifelse(project_id == pid, correct_date, sampling_date)) %>%
  select(-correct_date) %>%
  left_join(filled.table) %>%
...
res <- "Thomas Bygh Nymann Jensen <tbnj@bio.aau.dk>; Per Halkjær Nielsen <phn@bio.aau.dk>"
com <- "Sample MFD00398B removed as duplicate"
metadata.project <- paste0(pid, "/", pid,  "_metadata_summary.csv")
...
mfd_db <- mfd_db %>%
  ...
  filter(fieldsample_barcode != "MFD00398B")
...
com <- "Sample MFD00457B was removed as duplicate"
...
mfd_db <- mfd_db %>%
  ...
  mutate(mfd_areatype       = ifelse(project_id == pid, "Urban", mfd_areatype),
         ...
         habitat_typenumber = ifelse(project_id == pid & habitat_type == "city_parks", "4100", habitat_typenumber),
         habitat_typenumber = ifelse(project_id == pid & habitat_type == "city_other", "4200", habitat_typenumber),
  ...
  select(-correct_latitude,
         -correct_longitude) %>%
  filter(fieldsample_barcode != "MFD00457B")
...
filled.table <- read.xlsx(paste0(metadata.path, "/", pid, "/P06_3_filled_table.xlsx"), sheet=1) %>%
  mutate(correct_habitat_typenumber = habitat_typenumber,
         correct_date = ymd(sampling_date) %>% format("%Y-%m-%d")) %>%
  select(fieldsample_barcode,
         correct_habitat_typenumber,
         correct_date)
...
mfd_db <- mfd_db %>%
  ...
  mutate(project_id         = if_else(fieldsample_barcode %in% filled.table$fieldsample_barcode, pid, project_id),
         ...
         latitude           = if_else(project_id == pid & !is.na(correct_latitude), correct_latitude, latitude), # Priority to UTM-based coords
         longitude          = if_else(project_id == pid & !is.na(correct_longitude), correct_longitude, longitude), # Priority to UTM-based coords
...
com <- "10 samples are reallocared to P08_8: Natual - Sinks. Sample MFD06328 has been removed due to mislabelling."
...
filled.table <- readr::read_delim(file = paste0(metadata.path, "/", pid, "/230524_SustainScapes_MFD_HabitatClasses_ForVibeke.txt"),
  ...
  mutate(fieldsample_barcode = CORRECTED_barcode_fieldsample,
         ...
         sampling_date = if_else(str_length(sampling_date)>=20,
                                 ymd_hms(sampling_date, truncated = 3) %>% format("%Y-%m-%d"),
                                 dmy(sampling_date) %>% format("%Y-%m-%d")),
  ...
  select(fieldsample_barcode,
         ...
         correct_habitat_typenumber,
         correct_sitename) %>%
  filter(!fieldsample_barcode%in%c("MFD06200", "MFD10197"))
...
filled.table1 <- (readr::read_delim(file = paste0(metadata.path, "/", pid, "/230524_SustainScapes_MFD_HabitatClasses_ForVibeke.txt"),
  ...
  add_ontology(ontology_path = ontology.path) %>%
  mutate(sampling_date = if_else(str_length(sampling_date)>=20,
                                 ymd_hms(sampling_date, truncated = 3) %>% format("%Y-%m-%d"),
                                 dmy(sampling_date) %>% format("%Y-%m-%d")))

mfd_db <- mfd_db %>%
  filter(fieldsample_barcode!="MFD06328") %>%
  ...
  left_join(filled.table,
            by = c("fieldsample_barcode")) %>%
  mutate(project_id = if_else((fieldsample_barcode %in% filled.table$fieldsample_barcode), "P08_5", project_id),
  ...
  select(-correct_latitude,
         ...
         -correct_habitat_typenumber) %>%
  rbind(filled.table1) %>%
  mutate(mfd_sampletype = if_else(project_id == pid & is.na(mfd_sampletype), "Soil", mfd_sampletype),
         mfd_areatype = if_else(project_id == pid & is.na(mfd_areatype), "Natural", mfd_areatype)) %>%
  add_ontology(ontology_path = ontology.path) %>%
  filter(!fieldsample_barcode %in% c("MFD10676", "MFD06328", "MFD06329", "MFD10618")) %>% # Duplicated samples as of 2023.06.07
  mutate(fieldsample_barcode = if_else(fieldsample_barcode=="MDF06762", "MFD06762", fieldsample_barcode))
...
filled.table1 <- readxl::read_excel(paste0(metadata.path, "/", pid, "/P08_6_minimal_metadata_batch1&2.xlsx")) %>%
  ...
  mutate(correct_date = as.character(ymd(sampling_date)),
         correct_sitename = sitename,
         correct_habitat_typenumber = habitat_typenumber) %>%
  select(fieldsample_barcode,
         correct_date,
...
peo <- "Signe Normand <signe.normand@bio.au.dk>; Urs Treier <urs.treier@bio.au.dk>; Bjarke Madsen <bjarke.madsen@bio.au.dk>"
res <- "Per Halkjær Nielsen <phn@bio.aau.dk>; Thomas Bygh Nymann Jensen <tbnj@bio.aau.dk>"
com <- "Samples reallocared from P08_5: Natual - Sustain scapes. Removing the 10 samples form P08_5 that were moved to P08_8."
...
mfd_db <- mfd_db %>%
  rbind(filled.table) %>%
  add_ontology(ontology_path = ontology.path) %>%
  filter(!(fieldsample_barcode%in%filled.table$fieldsample_barcode & project_id=="P08_5"))
...
mfd_db <- mfd_db %>%
  ...
  mutate(habitat_typenumber = if_else(project_id == pid & mfd_areatype == "Natural" & mfd_sampletype == "Sediment" & habitat_typenumber == "3103", "3150", habitat_typenumber),
         ...
         habitat_typenumber = if_else(project_id == pid & mfd_areatype == "Natural" & mfd_sampletype == "Sediment" & habitat_typenumber == "1150", "3170", habitat_typenumber)) %>%
  add_ontology(ontology_path = ontology.path)
...
res <- "Per Halkjær Nielsen <phn@bio.aau.dk>"
com <- "Sample MFD00228 was removed because it was a duplicate"
...
mfd_db <- mfd_db %>%
  ...
  select(-correct_sitename) %>%
  add_ontology(ontology_path = ontology.path) %>%
  filter(fieldsample_barcode!="MFD00228")
...
filled.table <- readxl::read_excel(paste0(metadata.path, "/", pid, "/P12_5_frde_standard table.xlsx")) %>%
  mutate(project_id = pid,
    ...
    mfd_sampletype = str_replace(mfd_sampletype, "^\\w{1}", toupper),
         mfd_areatype = str_replace(mfd_areatype, "^\\w{1}", toupper),
    received = NA,
    longitude = if_else((project_id == pid & habitat_typenumber == "1211"), NA, as.numeric(longitude)),
    latitude = if_else((project_id == pid & habitat_typenumber == "1211"), NA, as.numeric(latitude)),
    habitat_typenumber = ifelse((project_id == pid & habitat_typenumber == "1211"), "1210",  habitat_typenumber)) %>%
  add_ontology(ontology_path = ontology.path)

mfd_db <- mfd_db %>%
  rbind(filled.table[,colnames(mfd_db)]) %>%
  mutate(fieldsample_barcode = if_else(fieldsample_barcode=="MDF12931", "MFD12931", fieldsample_barcode))
...
mfd_db <- mfd_db %>%
  mutate(mfd_areatype       = ifelse(project_id == pid, "Urban", mfd_areatype),
         mfd_sampletype     = ifelse(project_id == pid, "Water", mfd_sampletype),
         habitat_typenumber = ifelse(project_id == pid, "2100", habitat_typenumber),
         sampling_date      = if_else(project_id == pid, received, sampling_date)) %>%
  add_ontology(ontology_path = ontology.path)
...
filled.table <- readxl::read_xlsx(paste0(metadata.path, "/", pid, "/P13_2_table_to_fill_Wextrasamples2.xlsx")) %>%
  filter(!fieldsample_barcode%in%mfd_db$fieldsample_barcode) %>%
  mutate(habitat_typenumber = ifelse(habitat_typenumber == "2210", "2100", habitat_typenumber),
         habitat_typenumber = ifelse(habitat_typenumber == "2211", "2200", habitat_typenumber),
         longitude = as.numeric(longitude),
         latitude = as.numeric(latitude),
         received = NA,
         sampling_comment = NA,
...
mfd_db <- mfd_db %>%
  mutate(mfd_areatype       = ifelse(project_id == pid, "Urban", mfd_areatype),
         mfd_sampletype     = ifelse(project_id == pid, "Water", mfd_sampletype),
         habitat_typenumber = ifelse(project_id == pid, "2200", habitat_typenumber),
         sampling_date      = if_else(project_id == pid, received, sampling_date)) %>%
  rbind(filled.table[!filled.table$fieldsample_barcode%in%mfd_db$fieldsample_barcode,colnames(mfd_db)]) %>%
  #filter(!(project_id==pid & grepl("negative control", sampling_comment))) %>%
  add_ontology(ontology_path = ontology.path)
...
mfd_db <- mfd_db %>%
  rbind(filled.table) %>%
  mutate(habitat_typenumber = ifelse((project_id == pid & habitat_typenumber == "2111"), "4100", habitat_typenumber),
         habitat_typenumber = ifelse((project_id == pid & habitat_typenumber == "2112"), "4200", habitat_typenumber),
         habitat_typenumber = ifelse((project_id == pid & habitat_typenumber == "2119"), "4300", habitat_typenumber)) %>%
  #rbind(filled.table[!filled.table$fieldsample_barcode%in%mfd_db$fieldsample_barcode,colnames(mfd_db)]) %>%
  #filter(!(project_id==pid & grepl("negative control", sampling_comment))) %>%
  add_ontology(ontology_path = ontology.path)
...
mfd_db <- mfd_db %>%
  mutate(mfd_areatype       = ifelse(project_id == pid, "Urban", mfd_areatype),
         mfd_sampletype     = ifelse(project_id == pid, "Sediment", mfd_sampletype),
         habitat_typenumber = ifelse(project_id == pid, "1299", habitat_typenumber),
         sampling_date      = if_else(project_id == pid, received, sampling_date)) %>%
  add_ontology(ontology_path = ontology.path)
...
filled.table <- readxl::read_excel(paste0(metadata.path, "/", pid, "/P20_1_frde_standard table.xlsx")) %>%
  mutate(project_id = pid,
         ...
         habitat_typenumber = case_when(habitat_typenumber == "wa_u_6310" ~ "wa_u_1310",
                                        habitat_typenumber == "so_u_6310" ~ "so_u_3100",
                                        habitat_typenumber == "se_u_6310" ~ "se_u_2100",
                                   .default = habitat_typenumber), 
    latitude = as.numeric(latitude),
    longitude = as.numeric(longitude),
    received = NA) %>%
...
filled.table <- readxl::read_excel(paste0(metadata.path, "/", pid, "/P25_1_frde_standard table.xlsx")) %>%
  mutate(correct_mfd_sampletype = case_when(habitat_type == "natural_soil" ~ "Soil",
         ...
         habitat_typenumber = case_when(habitat_typenumber == "9110" ~ "2200",
                                        habitat_typenumber == "9120" ~ "2300",
                                        .default = habitat_typenumber),
         correct_habitat_typenumber = as.character(habitat_typenumber),
         correct_date = dmy(sampling_date) %>% format("%Y-%m-%d")) %>%
...
## Remove projects not included

```{r, warning=T, echo=eval_debug, eval=T}
mfd_db <- mfd_db %>%
  filter(!project_id %in% c("P04_1", "P12_4"))
```

## Add centroids for P04_3 and P04_5

```{r, warning=T, echo=eval_debug, eval=T}
centroids.df <- readxl::read_xlsx(paste0(metadata.path, "/P04_5/centroids_seges.xlsx")) %>%
  filter(!is.na(MFD_id)) %>%
  mutate(correct_longitude = longitude,
         correct_latitude = latitude,
         fieldsample_barcode = MFD_id) %>%
  select(fieldsample_barcode, correct_longitude, correct_latitude)

mfd_db <- mfd_db %>%
  left_join(centroids.df,
            by = "fieldsample_barcode") %>%
  mutate(longitude = if_else((is.na(longitude)&!is.na(correct_longitude)), correct_longitude, longitude),
         latitude = if_else((is.na(latitude)&!is.na(correct_latitude)), correct_latitude, latitude)) %>%
  select(-correct_longitude, -correct_latitude)
```

## Add coordinates reliability column

```{r, warning=T, echo=eval_debug, eval=T}
mfd_db <- mfd_db %>%
  mutate(coords_reliable = if_else(project_id %in% c("P04_3", "P04_5", "P06_3"), "Masked", "Yes"))
```

## Comparison with Basemap04

### Download LU map

```{r download LU map, echo=F, message=F, warning=F, error=F, results='asis'}
if(!file.exists(paste0(downloads.path,"/Basemap04_2021/lu_agg_2021.tif"))){
  download.file(url = "https://envs.au.dk/fileadmin/envs/Hjemmeside_2018/Zip_filer/Basemap04_public_geotiff.zip",
                paste0(downloads.path, "/Basemap04_public_geotiff.zip"))
  unzip(paste0(downloads.path, "/Basemap04_public_geotiff.zip"), exdir = downloads.path)
}

if(!file.exists(paste0(downloads.path,"/Basemap04_2021/lu_agg_2021_lonlat.tif"))){
  tmp.rast <- rast(paste0(downloads.path,"/Basemap04_2021/lu_agg_2021.tif"))  %>%
    project(crs(rast()))
  writeRaster(tmp.rast, paste0(downloads.path,"/Basemap04_2021/lu_agg_2021_lonlat.tif"),
              overwrite=T)
}

LU.rast <- rast(paste0(downloads.path,"/Basemap04_2021/lu_agg_2021_lonlat.tif"))

LU.codes <- cats(LU.rast)[[1]] %>%
  mutate(LU_summ = case_when(VALUE >= 110000 & VALUE <= 160000 ~ "Built_environment",
                             VALUE >= 211000 & VALUE <= 230000 ~ "Agriculture",
                             VALUE >= 311000 & VALUE <= 312000 ~ "Forest",
                             VALUE >= 321000 & VALUE <= 321000 ~ "Nature",
                             VALUE >= 322000 & VALUE <= 322000 ~ "Nature",
                             VALUE >= 321220 & VALUE <= 321220 ~ "Agriculture_Nature",
                             VALUE >= 322220 & VALUE <= 322220 ~ "Agriculture_Nature",
                             VALUE >= 411000 & VALUE <= 420000 ~ "Water",
                             VALUE >= 800000 & VALUE <= 800000 ~ "Unmapped",
                             VALUE >= 999999 & VALUE <= 999999 ~ "Germany"))

```

### Make SpatVector
```{r Make SpatVector, echo=F, message=F, warning=F, error=F, results='asis'}
sampling.locations <- vect(data.frame(lon = mfd_db$longitude, lat = mfd_db$latitude), geom = c("lon", "lat"))
```

### Extract overlay
```{r Extract overlay, echo=F, message=F, warning=F, error=F, results='asis'}
mfd_LU_ontology <- mfd_db %>%
  mutate(C_02 = (extract(LU.rast, sampling.locations) %>% pull(C_02) %>% as.character())) %>% # C_02 is written in the tif instead of C_01
  left_join(LU.codes, by = "C_02") %>%
  select(-VALUE) %>%
  select(project_id, fieldsample_barcode,
         latitude, longitude,
         mfd_sampletype, mfd_areatype, mfd_hab1, mfd_hab2, mfd_hab3,
         C_03, LU_summ)
```
...
### Concordance check
```{r Concordance check, echo=F, message=F, warning=F, error=F, results='asis'}
mfd_LU_ontology <- mfd_LU_ontology %>%
  mutate(to_check = case_when(is.na(mfd_hab1) ~ "No",
                              mfd_hab1 == "Biogas" ~ "No",
                              mfd_hab1 == "Bogs, mires and fens" & (LU_summ == "Nature" | LU_summ == "Water" | LU_summ == "Agriculture_Nature") ~ "No",
                              mfd_hab1 == "Coastal" & (LU_summ == "Nature" | LU_summ == "Water") ~ "No",
                              mfd_hab1 == "Drinking water" ~ "No",
                              mfd_hab1 == "Dunes" & (LU_summ == "Nature" | LU_summ == "Water") ~ "No",
                              mfd_hab1 == "Dunes" & project_id %in% c("P01_1", "P08_1", "P08_5") ~ "No",
                              mfd_hab1 == "Fields" & (LU_summ == "Agriculture" | LU_summ == "Agriculture_Nature") ~ "No",
                              mfd_hab1 == "Forests" & (LU_summ == "Forest" | LU_summ == "Nature") ~ "No",
                              mfd_hab1 == "Freshwater" & (LU_summ == "Water") ~ "No",
                              mfd_hab1 == "Grassland formations" & (LU_summ == "Nature" | LU_summ == "Agriculture_Nature") ~ "No",
                              mfd_hab1 == "Greenspaces" & (LU_summ == "Built_environment" | LU_summ == "Unmapped") ~ "No",
                              mfd_hab1 == "Other" ~ "No",
                              mfd_hab1 == "Roadside" & (LU_summ == "Built_environment" | LU_summ == "Unmapped") ~ "No",
                              mfd_hab1 == "Rocky habitats and caves" ~ "No",
                              mfd_hab1 == "Saltwater" & (LU_summ == "Water") ~ "No",
                              mfd_hab1 == "Sclerophyllus scrub" & (LU_summ == "Nature" | LU_summ == "Agriculture_Nature") ~ "No",
                              mfd_hab1 == "Temperate heath and scrub" & (LU_summ == "Nature" | LU_summ == "Agriculture_Nature") ~ "No",
                              mfd_hab1 == "Urban" & (LU_summ == "Built_environment" | LU_summ == "Unmapped") ~ "No",
                              mfd_hab1 == "Wastewater" ~ "No",
                              mfd_hab2 == "Semi-natural tall-herb humid meadows" & (LU_summ == "Agriculture_Nature" | LU_summ == "Agriculture") ~ "No",
                              mfd_hab2 == "Semi-natural dry grasslands" & (LU_summ == "Agriculture_Nature" | LU_summ == "Agriculture") ~ "No",
                              mfd_hab2 == "Salt marshes and salt meadows" & (LU_summ == "Agriculture_Nature") ~ "No",
                              mfd_hab3 == "Agricultural meadow (6430 subtype)" & (LU_summ == "Agriculture_Nature" | LU_summ == "Agriculture") ~ "No",
                              .default = "Yes"))
#mfd_LU_ontology %>%
#  filter(to_check=="Yes")
```

### Adjacent cells check - queen

#### Neighbour vect function
```{r Neighbour vect function, echo=F, message=F, warning=F, error=F, results='asis'}
get_neigbours <- function(raster, lon, lat, directions){
  spat.vect <- vect(data.frame(lon = lon,
                               lat = lat),
                    geom = c("lon", "lat"))
  cell.number <- cells(raster, spat.vect)[,2]
  neighbouring.cells <- adjacent(raster, cell.number, directions = directions) %>% c()
  neighbouring.values <- unlist(raster[neighbouring.cells])
  return(neighbouring.values)
}
```

#### Neighbourhood discordand cells
```{r Neighbourhood discordand cells1, echo=F, message=F, warning=F, error=F, results='asis'}
mfd_LU_ontology_neighbours1 <- mfd_LU_ontology %>%
  filter(to_check=="Yes") %>%
  rowwise() %>%
  mutate(LU_neighbours1 = left_join(data.frame(C_01=as.integer(as.character(get_neigbours(LU.rast,
                                                                                          lon = longitude,
                                                                                          lat = latitude,
                                                                                          "queen")))),
                                    LU.codes) %>%
...
#### Concordance check in neighbourhood
```{r Concordance check in neighbourhood1, echo=F, message=F, warning=F, error=F, results='asis'}
mfd_LU_ontology_neighbours1 <- mfd_LU_ontology_neighbours1 %>%
  mutate(to_check_neighbours1 = case_when(mfd_hab1 == "Biogas" ~ "No",
                                          mfd_hab1 == "Bogs, mires and fens" & ("Nature" %in% LU_neighbours1 | "Water" %in% LU_neighbours1 | "Agriculture_Nature" %in% LU_neighbours1) ~ "No",
                                          mfd_hab1 == "Coastal" & ("Nature" %in% LU_neighbours1 | "Water" %in% LU_neighbours1) ~ "No",
                                          mfd_hab1 == "Drinking water" ~ "No",
                                          mfd_hab1 == "Dunes" & ("Nature" %in% LU_neighbours1 | "Water" %in% LU_neighbours1) ~ "No",
                                          mfd_hab1 == "Fields" & ("Agriculture" %in% LU_neighbours1 | "Agriculture_Nature" %in% LU_neighbours1) ~ "No",
                                          mfd_hab1 == "Forests" & ("Forest" %in% LU_neighbours1 | "Nature" %in% LU_neighbours1) ~ "No",
                                          mfd_hab1 == "Freshwater" & ("Water" %in% LU_neighbours1) ~ "No",
                                          mfd_hab1 == "Grassland formations" & ("Nature" %in% LU_neighbours1 | "Agriculture_Nature" %in% LU_neighbours1) ~ "No",
                                          mfd_hab1 == "Greenspaces" & ("Built_environment" %in% LU_neighbours1 | "Unmapped" %in% LU_neighbours1) ~ "No",
                                          mfd_hab1 == "Other" ~ "No",
                                          mfd_hab1 == "Roadside" & ("Built_environment" %in% LU_neighbours1 | "Unmapped" %in% LU_neighbours1) ~ "No",
                                          mfd_hab1 == "Rocky habitats and caves" ~ "No",
                                          mfd_hab1 == "Saltwater" & ("Water" %in% LU_neighbours1) ~ "No",
                                          mfd_hab1 == "Sclerophyllus scrub" & ("Nature" %in% LU_neighbours1 | "Agriculture_Nature" %in% LU_neighbours1) ~ "No",
                                          mfd_hab1 == "Temperate heath and scrub" & ("Nature" %in% LU_neighbours1 | "Agriculture_Nature" %in% LU_neighbours1) ~ "No",
                                          mfd_hab1 == "Urban" & ("Built_environment" %in% LU_neighbours1 | "Unmapped" %in% LU_neighbours1) ~ "No",
                                          mfd_hab1 == "Wastewater" ~ "No",
                                          mfd_hab3 == "Agricultural meadow (6430 subtype)" & ("Agriculture" %in% LU_neighbours1 | "Agriculture_Nature" %in% LU_neighbours1) ~ "No",
                                          project_id %in% c("P05_1", "P05_2", "P16_4", "P18_2") & ("Built_environment" %in% LU_neighbours1 | "Unmapped" %in% LU_neighbours1) ~ "No",
                                          .default = "Yes"))
mfd_LU_ontology_neighbours1 %>%
  filter(to_check_neighbours1=="Yes") %>% nrow()
```

## Adjacent cells check - knight

### Neighbourhood discordand cells
```{r Neighbourhood discordand cells2, echo=F, message=F, warning=F, error=F, results='asis'}
mfd_LU_ontology_neighbours2 <- mfd_LU_ontology_neighbours1 %>%
  filter(to_check_neighbours1=="Yes") %>%
  rowwise() %>%
  mutate(LU_neighbours2 = left_join(data.frame(C_01=as.integer(as.character(get_neigbours(LU.rast,
                                                                                          lon = longitude,
                                                                                          lat = latitude,
                                                                                          "16")))),
                                    LU.codes) %>%
...
### Concordance check in neighbourhood
```{r Concordance check in neighbourhood2, echo=F, message=F, warning=F, error=F, results='asis'}
mfd_LU_ontology_neighbours2 <- mfd_LU_ontology_neighbours2 %>%
  mutate(to_check_neighbours2 = case_when(mfd_hab1 == "Biogas" ~ "No",
                                          mfd_hab1 == "Bogs, mires and fens" & ("Nature" %in% LU_neighbours2 | "Water" %in% LU_neighbours2 | "Agriculture_Nature" %in% LU_neighbours2) ~ "No",
                                          mfd_hab1 == "Coastal" & ("Nature" %in% LU_neighbours2 | "Water" %in% LU_neighbours2) ~ "No",
                                          mfd_hab1 == "Drinking water" ~ "No",
                                          mfd_hab1 == "Dunes" & ("Nature" %in% LU_neighbours2 | "Water" %in% LU_neighbours2) ~ "No",
                                          mfd_hab1 == "Fields" & ("Agriculture" %in% LU_neighbours2 | "Agriculture_Nature" %in% LU_neighbours2) ~ "No",
                                          mfd_hab1 == "Forests" & ("Forest" %in% LU_neighbours2 | "Nature" %in% LU_neighbours2) ~ "No",
                                          mfd_hab1 == "Freshwater" & ("Water" %in% LU_neighbours2) ~ "No",
                                          mfd_hab1 == "Grassland formations" & ("Nature" %in% LU_neighbours2 | "Agriculture_Nature" %in% LU_neighbours2) ~ "No",
                                          mfd_hab1 == "Greenspaces" & ("Built_environment" %in% LU_neighbours2 | "Unmapped" %in% LU_neighbours2) ~ "No",
                                          mfd_hab1 == "Other" ~ "No",
                                          mfd_hab1 == "Roadside" & ("Built_environment" %in% LU_neighbours2 | "Unmapped" %in% LU_neighbours2) ~ "No",
                                          mfd_hab1 == "Rocky habitats and caves" ~ "No",
                                          mfd_hab1 == "Saltwater" & ("Water" %in% LU_neighbours2) ~ "No",
                                          mfd_hab1 == "Sandfilter" ~ "No",
                                          mfd_hab1 == "Sclerophyllus scrub" & ("Nature" %in% LU_neighbours2) ~ "No",
                                          mfd_hab1 == "Temperate heath and scrub" & ("Nature" %in% LU_neighbours2) ~ "No",
                                          mfd_hab1 == "Urban" & ("Built_environment" %in% LU_neighbours2 | "Unmapped" %in% LU_neighbours2) ~ "No",
                                          mfd_hab1 == "Wastewater" ~ "No",
                                          mfd_hab3 == "Agricultural meadow (6430 subtype)" & ("Agriculture" %in% LU_neighbours2 | "Agriculture_Nature" %in% LU_neighbours2) ~ "No",
                                          project_id %in% c("P05_1", "P05_2", "P16_4", "P18_2") & ("Built_environment" %in% LU_neighbours2 | "Unmapped" %in% LU_neighbours2) ~ "No",
                                          project_id %in% c("P04_3", "P04_5", "P06_3") ~ "No", # Projects with confounded coordinates
                                          .default = "Yes"))

mfd_LU_ontology_neighbours3 <- mfd_LU_ontology_neighbours2 %>%
  filter(to_check_neighbours2=="Yes") %>% arrange(desc(project_id))
mfd_LU_ontology_neighbours2 %>%
  filter(to_check_neighbours2=="Yes") %>% arrange(desc(project_id)) %>% nrow()
```

### Manual checks
```{r Manual checks, echo=F, message=F, warning=F, error=F, results='asis'}
mfd_LU_ontology_neighbours3 <- mfd_LU_ontology_neighbours3 %>%
  mutate(to_check_neighbours3 = case_when(project_id %in% c("P05_1", "P05_2", "P16_4", "P18_2") & (LU_summ == "Built_environment" | LU_summ == "Unmapped") ~ "No",
                              project_id == "P12_1" ~ "No", # Deep sediment in the sea (out of mapped area)
                              mfd_areatype == "Subterranean" ~ "No", # Surface pixel is not relevant
                              fieldsample_barcode %in% c("MFD13532", "MFD13531") ~ "No", # Spring/spingfen checked on Google maps
                              project_id %in% c("P17_1") ~ "No", # Greenspaces double-checked in Google maps (MFD06900 and MFD06901 appear to be more natural than urban)
                              #fieldsample_barcode %in% c("MFD09779", "MFD09985", "MFD09987") ~ "Yes", # Water and sediment samples from nearby fjord but coords registered on land - hence wrong. change for MfD2
                              fieldsample_barcode == "MFD03535" ~ "Yes", # P11_1 - Sediment samples from nearby cost but coords registered on land
                              project_id %in% c("P11_1") ~ "No", # Saltwater sediments double-checked in Google maps (MFD03536 is the only odd one of the series)
                              fieldsample_barcode == "MFD03531" ~ "Yes", # P10_3 - Freshwater sediment from costal river but sample is on a street inland
                              project_id %in% c("P10_3") ~ "No", # Freshwater sediments double-checked in Google maps (MFD03531 is the only odd one of the series)
                              project_id %in% c("P10_1", "P10_2") ~ "No", # Running freshwater sediments from small streams, impossible to tell from Google Maps
                              project_id %in% c("P09_1") ~ "No", # Standing freshwater sediments, some case are obviously right from Google Maps, other harder to tell - the source might not be there every year or with the same boundaries
                              fieldsample_barcode %in% c("MFD10791", "MFD10907", "MFD10815", "MFD10771", "MFD10840") ~ "Yes", # Untouched forests samples that are not forests or in Norway
                              project_id %in% c("P08_5") ~ "No", # The ambiguous samples were checked manually on Google Maps
                              fieldsample_barcode %in% c("MFD03452") ~ "Yes", # Sample is not located in a natural habitat but on an agricultural field
                              fieldsample_barcode %in% c("MFD00832", "MFD02840", "MFD02841", "MFD03699", "MFD03701", "MFD03702", "MFD03800") ~ "Yes", # P08_3 - Samples with locations not matching description from project P08_3
                              project_id %in% c("P08_3") ~ "No", # Various samples checked on Google Maps, wrong placements above
                              project_id %in% c("P08_2") ~ "No", # The ambiguous samples were checked manually on Google Maps
                              fieldsample_barcode %in% c("MFD00263") ~ "Yes", # P01_1 - Samples should be a Molinia meadow but it is located in a field
                              project_id %in% c("P01_1") ~ "No", # Various samples checked on Google Maps, wrong placements above
                              fieldsample_barcode %in% c("MFD00449", "MFD00564") ~ "Yes", # P01_2 - Samples with locations not matching description from project P01_2 (instead they are fields, plantation)
                              project_id %in% c("P01_2") ~ "No", # Various samples checked on Google Maps, wrong placements above
                              project_id %in% c("P02_1") ~ "No", # Various samples checked on Google Maps, some samples have imprecise habitat
                              project_id %in% c("P03_1") ~ "No", # Various samples checked on Google Maps, some samples have imprecise habitat
                              project_id %in% c("P04_6") ~ "No", # Various samples checked on Google Maps
                              fieldsample_barcode %in% c("MFD10082") ~ "Yes", # Samples with locations not matching description from project P04_7 (field mapping on a house)
                              project_id %in% c("P04_8") ~ "No", # Various samples checked on Google Maps
                              project_id %in% c("P05_1") ~ "No", # Standing freshwater sediments, some case are  hard to tell - the source might not be there every year or with the same boundaries
                              fieldsample_barcode %in% c("MFD00594", "MFD00750", "MFD00863", "MFD01025", "MFD01053",
                                                         "MFD01116", "MFD01118", "MFD01310", "MFD01470", "MFD02568",
                                                         "MFD04531", "MFD09617") ~ "Yes", # P08_1 - Samples are located in impossible areas (e.g. in the middle of the sea or in Cameroon)
                              fieldsample_barcode %in% c("MFD03252", "MFD03253", "MFD03254", "MFD03255", "MFD03256",
                                                         "MFD03257", "MFD03258", "MFD03259", "MFD03260", "MFD03261") ~ "No", # P18_1 - Samples from harbours
                              project_id == "P05_2" ~ "No", # Leftover P05_2 urban freshwater bodies checked manually on Google Maps (change for mfd2)
                              project_id == "P11_2" ~ "No", # Leftover P11_2 fjords checked manually on Google Maps (change for mfd2)
                              project_id == "P11_3" ~ "No", # Leftover P11_3 fjords checked manually on Google Maps (change for mfd2)
                              project_id == "P08_6" ~ "No", # Leftover P08_6 forests checked manually on Google Maps (change for mfd2)
                              project_id == "P09_2" ~ "No", # Leftover P09_2 stanging freshwater bodies checked manually on Google Maps (change for mfd2)
                              project_id %in% c("P08_1") ~ "No", # Various samples checked on Google Maps, wrong placements above
                                          .default = "Yes"))

to_flag <- mfd_LU_ontology_neighbours3 %>%
  filter(to_check_neighbours3=="Yes") %>% arrange(desc(project_id)) %>% pull(fieldsample_barcode)
mfd_LU_ontology_neighbours3 %>%
  filter(to_check_neighbours3=="Yes") %>% arrange(desc(project_id)) %>% nrow()
```
...
```{r, warning=T, echo=eval_debug, eval=T}
mfd_db <- mfd_db %>%
  mutate(coords_reliable = if_else(fieldsample_barcode %in% to_flag, "No", coords_reliable))
```

## Sitename whitespace removal

```{r, warning=T, echo=eval_debug, eval=T}
mfd_db <- mfd_db %>%
  mutate(sitename = trimws(sitename, "l"),
         sitename = trimws(sitename, "r"))
```

## Add reference grid mapping

### Load grids

```{r, warning=T, echo=eval_debug, eval=T}
dk.1km <- vect(paste0(downloads.path, "/eea_v_3035_1_km_eea-ref-grid-dk_p_2013_v02_r00/dk_1km.shp")) %>%
  project("+proj=longlat +datum=WGS84")
dk.10km <- vect(paste0(downloads.path, "/eea_v_3035_10_km_eea-ref-grid-dk_p_2013_v02_r00/dk_10km.shp")) %>%
  project("+proj=longlat +datum=WGS84")
```

### Make vector of sampling locations
...
print_internal_map(data_table = mfd_projects, fname = paste0(releases.path, "/latest_mfd_projects.xlsx"))
```
...
writexl::write_xlsx(mfd_db[,release_indices], path = paste0(releases.path, "/latest_mfd_db.xlsx"))
```