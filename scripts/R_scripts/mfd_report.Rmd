...
com <- "Publications: So far unclear if we should make dedicated joint publication on this dataset."
...
res <- "Per Halkjær Nielsen <phn@bio.aau.dk>"
com <- "Close project – data cannot be used. Remove data from database – cannot be released."
...
res <- "Per Halkjær Nielsen <phn@bio.aau.dk>"
com <- "Publications:  Andrew has one in preparation"
...
res <- "Per Halkjær Nielsen <phn@bio.aau.dk>"
com <- "OBS: Date is estimated from a report that mentions that samples are taken in february. https://www.landbrugsinfo.dk/public/f/2/f/godskning_kvalstofprognosen_2018. Publications: So far unclear if we should make dedicated joint publication on this dataset."
...
mapping.project <- readxl::read_excel(paste0(metadata.path, "/", pid, "/mapping.xlsx")) %>%
  filter(!is.na(MFD_id)) %>%
  select(`Kvnet Punktnr`, MFD_id) %>%
  as.data.frame()

mfd_db <- mfd_db %>%
  #tmp <- mfd_db_subproject %>%
  ...
  mutate(mfd_areatype       = ifelse(project_id == pid & habitat_type == "agriculture", "Agriculture", mfd_areatype),
         ...
         #sampling_date      = if_else(project_id == pid & received == "2020-10-30", "2020-02-14", sampling_date),
         #sampling_date      = if_else(project_id == pid & received == "2021-03-15", "2021-02-14", sampling_date)) %>%
         sampling_date = received) %>%
  add_ontology(ontology_path = ontology.path) %>%
  select(-kvnet_hab) %>%
...
peo <- ""
res <- "Per Halkjær Nielsen <phn@bio.aau.dk>"
com <- "Publications: none – dataset is just part of MfD"
...
res <- "Per Halkjær Nielsen <phn@bio.aau.dk>"
com <- "Publications: It is likely that we could make dedicated joint publication on this dataset. "
...
res <- "Per Halkjær Nielsen <phn@bio.aau.dk>"
com <- "Publications: This will be part of AU collaborations with joint publication on this dataset"
...
res <- "Per Halkjær Nielsen <phn@bio.aau.dk>"
com <- "No publications. We have promised to send an overview of microbes in the samples - to look for potential major differences."
...
com <- "In theory the dried sampels are not sediment, but soil samples. Does not fit the ontology scheme. Publication: Agreement on one joint paper on rainwater basins, metals and microbes. Dian S. driving force."
...
peo <- ""
res <- "Per Halkjær Nielsen <phn@bio.aau.dk>"
com <- "The responisble people are Erika Dvarionaite and Amalie Berg-Mortensen (email to be found). Publications: So far unclear if we should make dedicated publication on this dataset. Likely not. Could perhaps go together with data from all MfD lakes (P09_1)."
...
res <- "Per Halkjær Nielsen <phn@bio.aau.dk>"
com <- "No GPS on samples, estimated from sampling_comment where a city is mentioned. Project should be evaluated for future work."
...
res <- "Per Halkjær Nielsen <phn@bio.aau.dk>"
com <- "Should this project be closed? Method does not work well for the type of sample (came as DNA, extracted with FastSpin)."
...
res <- "Per Halkjær Nielsen <phn@bio.aau.dk>"
com <- "Publication: Data for several publications. Need to be discussed which papers and lead authors."
...
res <- "Per Halkjær Nielsen <phn@bio.aau.dk>"
com <- "Assuming dates are correct, but not sure. Publications: One paper is being written by Anne-Cathrine based on amplicon data."
...
peo <- ""
res <- "Per Halkjær Nielsen <phn@bio.aau.dk>"
com <- "Figure out what projects are collapsed into this project. Add description of project."
...
com <- "Publications: This will be part of AU collaborations with joint publication on this dataset. We only have GPS and date – are these correct? Sampling points look very clustered."
...
res <- "Per Halkjær Nielsen <phn@bio.aau.dk>; Thomas Bygh Nymann Jensen <tbnj@bio.aau.dk>"
com <- "Publications: This will be part of AU collaborations – likely with joint publication on this dataset and other forests. Check if the 107 samples and the new 250 are the same project (kvadratnet?) – see P23_1"
...
peo <- "Erik Aude <eau@habitatvision.dk>; Lene Thomsen <lt@habitatvision.dk>"
res <- "Per Halkjær Nielsen <phn@bio.aau.dk>"
com <- "Lakes sampled by Habitat Vision. Publication: No agreements – perhaps together with Urban_ponds (P05_2) + others on \"freshwaters\" (ponds, streams, highway)"
...
res <- "Per Halkjær Nielsen <phn@bio.aau.dk>"
com <- "Publications: Likely not, if so AU lead."
...
peo <- "Erik Aude <eau@habitatvision.dk>; Lene Thomsen <lt@habitatvision.dk>"
res <- "Per Halkjær Nielsen <phn@bio.aau.dk>"
com <- "Publications: Likely not. Perhaps with P05_2, P10_2 and other on \"freshwater\"."
...
res <- "Per Halkjær Nielsen <phn@bio.aau.dk>"
com <- "Publications: Potentially joint publication. Jeppe/Nadieh lead"
...
res <- "Per Halkjær Nielsen <phn@bio.aau.dk>"
com <- "Publications: Yes, joint publication. MOU is signed. AU lead."
...
com <- "The ANME part is in a separate project with deep sequencing (Caitlin). Samples received as DNA from phenol-chloroform extraction – worked really bad. Publications: joint pub on ANME (AU lead)"
...
com <- "Close project – don’t include in MFD."
...
res <- "Per Halkjær Nielsen <phn@bio.aau.dk>"
com <- "Publication: likely not. If so VIA lead. Very varying results for prep and sequencing. Sampling dates exist but have to be fetched."
...
res <- "Per Halkjær Nielsen <phn@bio.aau.dk>"
com <- "Publication: likely not. If so, DTU lead."
...
peo <- ""
res <- "Per Halkjær Nielsen <phn@bio.aau.dk>"
com <- "People Erika Dvarionaite and Amalie Berg-Mortensen (their emails need to be retrieved). Publication:  likely not as \"stand alone\" – perhaps part of larger soil paper?"
...
res <- "Per Halkjær Nielsen <phn@bio.aau.dk>"
com <- "Publication: Likely not. No metadata added yet."
