...
```{r, warning=T, echo=eval_debug, eval=T}
mfd_db <- mfd_db %>%
  mutate(coords_reliable = if_else(project_id %in% c("P04_3", "P04_5", "P06_3"), "No", "Yes"))
```
...
LU.rast <- rast(paste0(downloads.path,"/Basemap04_2021/lu_agg_2021.tif")) %>%
  project(crs(rast()))
...
mfd_LU_ontology <- mfd_db %>%
  mutate(C_01 = (extract(LU.rast, sampling.locations) %>% pull(C_01) %>% as.character()  %>% as.integer())) %>%
  left_join(LU.codes, by = "C_01") %>%
  select(-VALUE) %>%
...
mfd_LU_ontology_neighbours3 <- mfd_LU_ontology_neighbours3 %>%
  mutate(to_check_neighbours3 = case_when(project_id %in% c("P05_1", "P05_2", "P16_4", "P18_2") & (LU_summ == "Built_environment" | LU_summ == "Unmapped") ~ "No",
                              ...
                              fieldsample_barcode %in% c("MFD09779", "MFD09985", "MFD09987") ~ "Yes", # Water and sediment samples from nearby fjord but coords registered on land - hence wrong
