...
filled.table <- read.xlsx(paste0(metadata.path, "/", pid, "/P01_1_filled_table_TGF.xlsx"), sheet = 1, detectDates = T) %>%
...
res <- "Per Halkjær Nielsen <phn@bio.aau.dk>"
com <- "OBS: Date is estimated from a report that mentions that samples are taken in february. https://www.landbrugsinfo.dk/public/f/2/f/godskning_kvalstofprognosen_2018. Publications: So far unclear if we should make dedicated joint publication on this dataset. The sampling occurred over 12 days (3-14 Feb in 2020 and 1-12 Feb 2021) and in case the sampling date was missing we used the middle of the sampling period (8 and 6 Feb for 2020 and 2021 respectively). 14 samples have been confirmed coming from not carachterised fields."
...
mfd_db <- mfd_db %>%
  ...
  left_join((mapping.project %>%
               `colnames<-`(c("kvnet_punktnr", "fieldsample_barcode"))),
             by = "fieldsample_barcode") %>%
  mutate(sitename           = if_else(project_id == pid, paste0("kvnet_punktnr_", kvnet_punktnr), sitename)) %>%
  select(-kvnet_punktnr) %>%
  mutate(Year = year(received),
         Proevenr = str_remove(sitename, "kvnet_punktnr_")) %>%
  left_join(filled.table, by=c("Proevenr", "Year")) %>%
  mutate(sampling_date = case_when( project_id == pid & !is.na(Dato) ~ Dato,
                                    project_id == pid & is.na(Dato) & Year == 2020 ~ "2020-02-08",
                                    project_id == pid & is.na(Dato) & Year == 2021 ~ "2021-02-06",
...
utmcoor <- SpatialPoints(cbind(as.numeric(gsub("'|D", "", filled.table$Utm_x)),
                         ...
                         proj4string=CRS("+proj=utm +zone=32"))
...
filled.table <- readxl::read_excel(paste0(metadata.path, "/", pid, "/P08_6_filled_table.xlsx")) %>%
  mutate(correct_date = dmy(sampling_date) %>% format("%Y-%m-%d"),
